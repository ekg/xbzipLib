...
int count_row_mu (fm_index * index, uchar * pattern, ulong len,
		  ulong sp, ulong ep);

int fm_multi_count (fm_index * index, uchar * pattern,
		    ulong len, multi_count ** list);

int multi_locate (ulong sp, ulong element, ulong * positions, fm_index *);
 ...
 */
int
locate (void *indexe, uchar * pattern, ulong length, ulong ** occ,
	ulong * numocc)
{
	...
	*numocc = 0;
	...
	occs = *occ = (ulong *) malloc (sizeof (ulong) * (*numocc));
	if (*occ == NULL)
		return FM_OUTMEM;
 ...
 */
int
count (void *indexe, uchar * pattern, ulong length, ulong * numocc)
{
	fm_index * index = (fm_index *) indexe;
	multi_count *groups;
	int i, num_groups = 0;
 ...
 */
int
count_row_mu (fm_index * index, uchar * pattern, ulong len, ulong sp,
	      ulong ep)
{
	uchar chars_in[index->alpha_size];
	ulong occsp[index->alpha_size], occep[index->alpha_size];
	...
	while ((sp <= ep) && (len > 0))
	{
		...
		for(i=0; i<num_char; i++) {
			if (c==index->subchar) 
			if ((chars_in[i] == index->specialchar)&&(index->specialchar != index->subchar))
			{
				/*
				 * occorrenza subchar DIVIDI! 
			 	*/
				ssp = index->bwt_occ[index->specialchar] + occsp[index->specialchar]; 
				sep = ssp +(occep[index->specialchar] - occsp[index->specialchar]) - 1;
				count_row_mu (index, pattern, len, ssp, sep);
				if (find==1) break;
				find = 1;
			}
			if (chars_in[i] == c) {
 ...
 */
int
fm_multi_count (fm_index * index, uchar * pattern, ulong len,
		multi_count ** list)
{
	ulong sp, ep, i;
	uchar c;
	...
	for (i = 0; i < len; i++)
	{
		...
		if((pattern[i]==index->specialchar) && (pattern[i]!=index->subchar)) 
			{
				free(lista);
				/* inverse remap pattern  */
				int j;
	...
	/*
	 * Se l'ultimo carattere del pattern e' uguale al carattere sostituito 
	 * allora devi cercare occorrenze per il carattere con cui l'ha
	 * sostituito (specialchar) e per il carattere originale (subchar
	 * ..ricerca gia' eseguita) 
	 */

	if ((c == index->subchar) && (index->subchar != index->specialchar))
	{
		c = index->specialchar;
		sp = index->bwt_occ[c];
		if (c == index->alpha_size - 1)
		...
		count_row_mu (index, pattern, len - 1, sp, ep);	// ric
 ...
 */
static inline void
get_pos (ulong first_row, ulong element, suint step, ulong * pos,
	 	 fm_index * s)
{
	int offset, skipbits;
	ulong postext, i;
 ...
 */
int
multi_locate (ulong sp, ulong element, ulong * positions, fm_index * index)
{
	...
	ulong curr_row, used, recurs, elements, diff;
	ulong *elem_array;	// contiene il numero di elementi del sottogruppo
	ulong occsp[index->alpha_size];
	ulong occep[index->alpha_size];
	ulong occcharinf = index->bwt_occ[index->specialchar];
	ulong occcharsup;
	 ...
	 * per singola locate 
	 */
	ulong occ_sb[index->alpha_size], occ_b[index->alpha_size];
	suint localstep;
	uchar c, c_sb, cb;

	if(index->specialchar==index->alpha_size-1)
		occcharsup = index->text_size-1;
	else 
		occcharsup = index->bwt_occ[index->specialchar+1];
	...
	if ((sp >= occcharinf) && (sp+element-1 < occcharsup)) {
		get_pos (sp-occcharinf, element, 0, positions, index);
		return element;
	}
	
	step_array = malloc (sizeof (ulong) * element);
	elem_array = malloc (sizeof (ulong) * element);
	...
	while (recurs < element)
	{
		...
		for (j = 0; j < num_char; j++)
		{
			...
			if (diff == 1)
			{
				...
				if (curr_row == index->bwt_eof_pos)
					continue;
				get_pos (curr_row - occcharinf, 1, localstep,
   ...
   while (j <=  s->text_size - length) {
      ...
      if (i < 0) {
		 ...
		 if(*numocc == alloc) {
			 	...
			 	*occ = realloc(*occ, sizeof(ulong)*alloc);
   				if(*occ == NULL) 
						return FM_OUTMEM;   
			}
		 (*occ)[*numocc-1] = j;
         ...
         j += MAX((ulong) bmGs[i], bmBc[s->text[i + j]] - length + 1 + i);
   }
   if(*numocc>0) occ = realloc(*occ, sizeof(ulong)*(*numocc));
   else occ = NULL;
   return FM_OK;
}
