...
int compress_superbucket(fm_index *s, ulong num);
...
typedef struct { // Solo per i test: memorizza spazio occupato
	ulong bucket_compr; 
	ulong bucket_occ;
	ulong bucket_alphasize;
	ulong bucket_pointer;		
	ulong sbucket_occ;       	
	ulong sbucket_alphasize; 	
	ulong prologue_size;
	ulong sbucket_bitmap;
	ulong bucket_bitmap;
	ulong marked_pos;
	ulong temp;
} measures;
...
int fm_build_config (fm_index *index, suint tc, double freq, 
	...
	bsl1 = bsl1<<10;
	/* Checks */
	if ( (freq > 1) || (freq < 0) ) return FM_CONFERR;				  
	...
	if (freq > 0.5) freq = 1;
	if (freq == 0) index->skip =0;
	else index->skip = (suint) 1/freq;	/* 1/Mark_freq 2% text size */
	return FM_OK;
}
...
*/
int build_index(uchar *text, ulong length, char *build_options, void **indexe) {
...
int fm_build(fm_index *index, uchar *text, ulong length) {
	...
	ulong stima_len_compress = 100 + index->text_size*1.5;
	
	stima_len_compress += index->num_marked_rows*sizeof(ulong); 
	...
	#if TESTINFO
	fprintf(stderr, "Index size %lu Kb Text Size %lu Kbyte log2 text size %u\n", index->compress_size/1024, index->text_size/1024, index->log2textsize);
	fprintf(stderr, "Alphasize %d\n\n", index->alpha_size);
	fprintf(stderr, "Prologue size %lu Kb\n\n", Test.prologue_size/1024);
	fprintf(stderr, "Buckets size %lu # Buckets %lu\n", index->bucket_size_lev2, index->num_bucs_lev2);
...
/*
	La funzione seleziona un carattere da sostituire.
	L'obiettivo e' quello di trovare un carattere sufficientemente 
	distribuito sul testo. 
	Il carattere scelto viene sostituito da un carattere non presente
	nel testo. 
	Il testo viene modificato.
*/
static int select_subchar(fm_index *s) {

	ulong skipmezzi = s->skip>>1;
	ulong i, dx, sx, j;
	ulong distribuzione[ALPHASIZE]; /* numero di blocchetti in cui compare il carattere sul testo */
	ulong num_occ[ALPHASIZE]; /* numero occorrenze del carattere sul testo */
	suint mappa[ALPHASIZE];	  /* caratteri che occorrono nel blocchetto */ 
	suint trovato = 0;
	
	//if (s->text_size <= s->skip) return FM_GENERR;
	if (s->skip == 1) {
	    s->num_marked_rows  = s->text_size;
		return FM_OK;
		}
	if (s->skip == 0)  {
		s->num_marked_rows = 0; s->specialchar = s->subchar = 0;
		return FM_OK;
		}
	if (s->text_size <= s->skip) {
		s->skip = s->text_size/2;
		s->subchar = s->specialchar = s->text[s->skip];
		s->num_marked_rows = 0;
		for(i=0;i<s->text_size;i++) 
			if(s->text[i]==s->specialchar) s->num_marked_rows++;
		return FM_OK;		
	} 
	
	s->num_marked_rows = (ulong) ceil((s->text_size+s->skip-1)/s->skip);

	/* Inizializzazione array */
	for (i=0;i<ALPHASIZE;i++) 
		mappa[i] = distribuzione[i] = num_occ[i] = 0;
	
	/* Si divide il testo in blocchetti di s->skip caratteri 
	   e per ogni carattere i che occorre nel blocchetto se ne tiene conto 
	   in distribuzione.
	   Conta inoltre le occorrenze dei caratteri nel testo.
	   Non partire da 0 tanto c'e' bwt_eof ad indicare la posizione 0.
	   Questa e' una euristica che puo' essere migliorata.
	*/
	
	for (i=skipmezzi;i<s->text_size;i+=s->skip) {
		for(j=i;j<MIN(i+s->skip, s->text_size);j++) { // vedi distribuzione nel pezzetto
			if (mappa[s->text[j]] == 0) {
							mappa[s->text[j]] = 1;
							distribuzione[s->text[j]]++;
			}
		num_occ[s->text[j]]++; /* conta occ totali */
		}
	
		for (j=0;j<ALPHASIZE;j++) /* nuovo blocchettino */
			mappa[j]= 0;
	}
	
	
	/* Il carattere scelto da sostituire e' 
		- quello piu' distribuito 
		- a parita' di distribuzione quello che ha piu' occorrenze 
	*/	
	s->subchar = 0;
	for(i=1; i<ALPHASIZE; i++) {
			if (distribuzione[i] > distribuzione[s->subchar]) 
					s->subchar = i; /* il carattere che si vuole sostituire */
			if ( (distribuzione[i] == distribuzione[s->subchar]) &&
				 (num_occ[i] > num_occ[s->subchar]))  s->subchar = i; // a parita'  
			}
	
	//printf("distribuzione carattere scelto %lu\n text/s-skip marcature attese %lu\n percentuale marcatura %.2f%\n carattere scelto numero %d\n numero occ carattere %lu\n",
	//		distribuzione[s->subchar], s->text_size/s->skip,
	//		distribuzione[s->subchar]/(s->text_size*0.01), s->subchar, num_occ[s->subchar]);
	...
	/* Due casi particolari 
	   - Se il numero di caratteri da marcare e' > del numero di occ del carattere 
	   	 sostituito.
	   - o se il testo ha 256 caratteri 
	   Non opero la sostituzione e il carattere sostituito e' uguale al 
	   carattere sostituito.
	   Di fatto ho forzato il numero di righe marcate.
	*/
	if ( (s->num_marked_rows > num_occ[s->subchar]) || (i == ALPHASIZE)) {
			s->num_marked_rows = num_occ[s->subchar];
			s->specialchar = s->subchar;
			if(i==ALPHASIZE) 
					fprintf(stderr,"Warning: This file contains 256 distinct chars, you may experience a bad compression...\n");
			s->skip = 2;
			return FM_OK;
	}
		
	/* Poche occorrenze di s->subchar non e' conveniente sostituire */
	if (s->num_marked_rows >= (num_occ[s->subchar]*0.9)) {
			s->num_marked_rows = num_occ[s->subchar];
			s->specialchar = s->subchar;
			return FM_OK;
	}

	/* Opera la sostituzione cercando di sostituire in maniera piu' 
	   distribuita sul testo.
	   Andando a destra e sinistra dal centro del blocchetto corrente 
	*/
	s->num_marked_rows = 0;
	for(i = s->skip; i<s->text_size;) {
			if (s->text[i]== s->subchar) {
				s->text[i] = s->specialchar;
				i += s->skip;
				s->num_marked_rows++;
			} else {
				trovato = 0; sx =i-1; dx = i+1; j=0;
				while(j<=skipmezzi) {
					if(dx<s->text_size) { // se non sono oltre  
						if (s->text[dx] == s->subchar) {
							s->text[dx] = s->specialchar;
							i = dx + s->skip; 
							trovato = 1;
							s->num_marked_rows++;
							break;
				    ...
				    if (s->text[sx] == s->subchar) {
						s->text[sx] = s->specialchar;
						i = sx + s->skip; 
						trovato = 1;
						s->num_marked_rows++;
						break;
				...
				if (!trovato) i+=skipmezzi;
			}
		}
	

	#if TESTINFO
	printf("Distribuzione caratteri text_size/s->skip = %lu\n",s->text_size/s->skip);
    printf("Distribuzione %lu carattere scelto %d '%c'\n",distribuzione[s->subchar],s->subchar,s->subchar);
	
	ulong prev = 0, maxdist = 0, mindist = s->text_size, where = 0, totdist = 0;
	
	for(i=0;i<s->text_size;i++) {
		if (s->text[i]==s->specialchar) {
			if((i-prev)>maxdist) {maxdist = i-prev; where=prev;}
			if((i-prev)<mindist) mindist = i-prev;
			totdist += (i-prev);
			prev = i;
			}
		}
	if((i-prev)>maxdist) {maxdist = i-prev; where=prev;}
	if((i-prev)<mindist) mindist = i-prev;
	printf("Massima distanza tra due posizioni marcate %lu\n", maxdist);
	printf("Minima distanza tra due posizioni marcate %lu\n", mindist);
	printf("Numero righe marcate %lu\n", s->num_marked_rows);
	#endif	
...
int build_bwt(fm_index *s) {
  ...
  */
  ...
  ulong *sa = s->lf;		 	 // punta al primo elemento 
...
int compute_locations(fm_index *s) { 
	...
	ulong i,j; /* numero posizioni marcate */
	uchar spchar = s->char_map[s->specialchar];
	ulong firstrow = s->pfx_char_occ[spchar];
...
int compute_info_superbuckets(fm_index *s)
{
  ...
  ulong currentBuck = 0;  // indice superbuckets
  ulong dim = s->bucket_size_lev1; // per non dividere
...
void write_prologue(fm_index *s) {
  ...
  /* write Mark Mode & starting byte of occ-list */
  ...
  for(i=0;i<s->num_bucs_lev1;i++) {
	...
	fm_bit_flush();
  	if(i>0) // write prefix-occ
		...
		dest = memcpy(dest, sb.occ, sizeof(ulong) * s->alpha_size);
	  	s->compress_size += sizeof(ulong) * s->alpha_size;
...
int compress_superbucket(fm_index *s, ulong num)
{
  ...
  ulong k, temp, sb_start, sb_end, start,len, bocc[ALPHASIZE], b2,i,j;
  int is_odd;
...
void write_locations(fm_index *s) {
	...
	for(i=0; i<s->num_marked_rows;i++) 
		fm_bit_write(s->log2textsize, s->loc_occ[i]);
