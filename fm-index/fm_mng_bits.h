...
ulong * __Num_Bytes;      /* numero byte letti/scritti */
...
void fm_init_bit_writer(uchar *, ulong *);
...
void fm_bit_write(int, ulong); // (numero bits, intero da codificare) 
void fm_bit_write24(int bits, ulong num);
int fm_bit_read(int);
...
void fm_uint_write(ulong);
ulong fm_uint_read(void);
...
ulong fm_integer_decode(unsigned short int);
