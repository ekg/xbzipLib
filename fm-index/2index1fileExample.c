...
int main(int argc, char *argv[]) {
	...
	ulong toc_size = 2*sizeof(ulong);
	uchar * compress = (uchar *) malloc(sizeof(uchar)*(index_size1+index_size2+toc_size));
	
	// save TOC. the 2 index_size
	memcpy(compress, &index_size1, sizeof(ulong));
	memcpy(compress+sizeof(ulong), &index_size2, sizeof(ulong));
	...
	//Read sizes
	memcpy(&index_size1, compress, sizeof(ulong));
	memcpy(&index_size2, compress+sizeof(ulong),sizeof(ulong));
	printf("index size %lu %lu\n", index_size1, index_size2);
	...
	//print the texts
	printf("FILE %s\n", infile1);
	//fwrite(text1, sizeof(uchar), text_len1, stderr);
	printf("\n\n\nFILE %s\n", infile2);
