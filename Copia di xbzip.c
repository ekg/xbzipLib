...
int main(int argc, char **argv) {
  ...
  int fd = -1;
  FILE *outfile; 
  UChar *ctext, *text, *tmp, *path_string, **path, *snippet;
  UInt32 text_len, ctext_len;
  int visualize, decompress, compress, compr_type, indexing, extracting, searching, printing;
  int first_row, last_row, i, path_len, num_occ, path_occ, startc, row2text, snippetLength;
  int printedRow;
 ...
 if (argc<2) {
    ...
    printf("xbzip [ -i [-l NUM1][-a NUMS] ] [-e] [-p row] [-s \"PATH\" [-w]] inFileName \n\n");
	printf("\t-i to index\n");
	...
	printf("Option -i generates a file with name inFileName.xbzi.\n\n");
  ...
  opterr=0;
  path_string=NULL;
  while ((c=getopt(argc, argv, "vwl:a:ip:es:c:d:o:")) != -1) {
    switch (c)
      {
        case 'v':
          Verbose++;  
  ...
  if ( (decompress + compress + extracting + indexing + searching + printing == 0) )
	  fatal_error("You must specify either (de)comression or (de)indexing or searching!\n");

  if ( (decompress + compress + extracting + indexing + searching + printing > 1) )
  ...
  if (optind<argc){
	...
	if ((extracting || searching || printing) && strcmp(tmp,"xbzi"))
  ...
  if( extracting ) {
		...
		stat(infile_name, &info); 
  		ctext_len = (UInt32) info.st_size;
		ctext = (UChar *) mmap(0, text_len, PROT_READ, MAP_SHARED, fd, 0) ;
